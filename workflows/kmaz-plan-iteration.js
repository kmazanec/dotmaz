...
1. RECONCILE every shared contract across all features into ONE frozen spec, consistent with the cited ADRs. For each contract, pre-commit EVERY feature's additive extension (new enum member / union variant / optional wire field) TOGETHER with every consumer that must stay exhaustive over it (every switch, validator, provider schema). A shared extensible type and the code that must handle all its cases are ONE contract — landing the members up front turns N late breaks into zero. Give each a minimum-viable frozen \`signature\` the build workflow will implement and commit before any feature work.
   COMPLETENESS CHECK — walk every feature's plan steps against the frozen signature and confirm each OPERATION the plan performs through the contract is actually expressible in it (a cleanup/rollback step needs a delete; a listing view needs a list; an update path needs an update). A frozen interface missing a method one feature's own approved plan requires blocks that feature mid-build (measured: a storage seam frozen without \`delete\` blocked the feature whose plan said "atomic upload with cleanup") — the builder is rightly forbidden from widening a frozen contract, so the gap MUST be caught here, not there.
